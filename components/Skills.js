          ...
          {Object.entries(skills).map(([category, items]) => (
            ...
            >
              ...
              <ul className="grid grid-cols-2 gap-3" role="list">
                {items.map(skill => (
                  <li key={skill} className="flex items-center gap-3 px-3 py-2 rounded-md bg-[#f1f8fb] dark:bg-gray-600">
                    <span className="flex-none" aria-hidden="true">{getIcon(skill)}</span>
                    <span className="text-sm font-medium text-gray-800 dark:text-gray-100">{skill}</span>
                  </li>
                ))}
              </ul>
            </section>
